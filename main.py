...
def get_user_detail(mid, cookies):
    img_key, sub_key = getWbiKeys()  # 获取最新的 img_key 和 sub_key
    wbi_params = {"mid": mid}
    signed_params = encWbi(wbi_params, img_key, sub_key)  # 使用WBI签名
    ...
    if response.status_code == 200:
        ...
        silence = "封禁" if data.get("silence") else "正常"
        return f"昵称: {name}, 等级: {level}, 是否封禁: {silence}"
    else:
        return "获取用户信息失败"
...
def output_result(danmu,cookies):
    """
    输出所有逆向后弹幕信息
    ...
    output(get_user_detail(danmu['mid'], cookies))
    output("==========================")
